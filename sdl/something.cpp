...
enum movper{MOVEMENT_WALKABLE = 0, MOVEMENT_BLOCKED = 1};
...
int* getClickedTile(){
	...
	tilepos[0] = (mouseposx + camposx) / renderedtilesize;
	tilepos[1] = (mouseposy + camposy) / renderedtilesize;
	return tilepos;
}
...
int getClickedSubtile(){
	int subtileposx = (mouseposx + camposx) / renderedsubtilesize;
	int subtileposy = (mouseposy + camposy) / renderedsubtilesize;
	int u;
...
void renderEntity(Entity *entity, SpriteSheet *spritesheet, SDL_Rect *srcrect, SDL_Rect *dstrect){
	...
	dstrect->x = entity->getXPos() * renderedtilesize - camposx;
	dstrect->y = entity->getYPos() * renderedtilesize - camposy;
	SDL_RenderCopy(renderer, spritesheet->getTexture(), srcrect, dstrect);
}
...
Uint32 callback(Uint32 interval, void *param){
	...
	if(camerafollowplayer){
		camposx = (player->getXPos() * renderedtilesize) - 368;
		camposy = (player->getYPos() * renderedtilesize) - 268;
	}
